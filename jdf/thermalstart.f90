...
! Modified by K.X. Zhang 04/12/2022
!
! Y.Y. Ruan 12/19/2016 
...
!   Edited shear modulus; Added dry peridotite solidus (Katz, 2003)
!
! K.X. Zhang 03/13/2023
!   Added linear depletion solidus
!=======================================================
  ...
  real, parameter :: pottemp = 1325. ! potential temperature (c)
  ...
  public :: predotite_solidus_Katz2003
  public :: predotite_wetsolidus_Hirschmann2000
  public :: linear_depletion
  public :: simple_depletion
  public :: simple_water
  public :: linear_fit
  ...
  contains
    ...
    subroutine predotite_wetsolidus_Hirschmann2000(pressure, temperature)
      ...
      real, parameter :: coeff_b = 92.5    ! C/GPa  !!! by Ruan
      real, parameter :: coeff_c = 1120.7  ! C
    ...
    subroutine predotite_solidus_Katz2003(pressure,temperature)
      implicit none
      real,parameter :: A1=1085.7, A2=132.9, A3=-5.1
      real :: pressure, temperature ! C, Gpa

      temperature = A1 + A2*pressure + A3*pressure*pressure

      return
    end subroutine

    subroutine linear_depletion(pressure,temperature)
      implicit none
      real,parameter :: max_change=150 ! Celsius
      real,parameter :: begin_depth=70 ! km
      real,parameter :: complete_depth=10 ! km
      real :: pressure,temperature ! C,gpa
      real :: temperature1,depth
      
      call predotite_solidus_Katz2003(pressure,temperature1)
      
      depth = pressure*1e+3/(3.2 * 9.8)
      
      if (depth .gt. 10 .and. depth .lt. 70) then
        temperature = temperature1+(begin_depth-depth)/(begin_depth-complete_depth)*max_change
      else if (depth .le. 10) then
        temperature = temperature1 + max_change
      else
    ...
    subroutine linear_fit(pressure,temperature)
    implicit none
    real,parameter :: max_w=180 ! C
    real,parameter :: min_w=80 ! C
    real,parameter :: begin=150 ! km
    real,parameter :: bulk=70 ! km
    real :: pressure,temperature
    real :: depth,temperature1
    
    depth = pressure*1e+3/(3.2 * 9.8)
    
    if (depth .lt. bulk) then
      call linear_depletion(pressure,temperature1)
      temperature = temperature1
    else 
      call predotite_solidus_Katz2003(pressure,temperature1)
      temperature =temperature1+(begin-depth)/(begin-bulk)*(max_w-min_w)-max_w
    end if
    return 
    end subroutine

       
    subroutine simple_depletion(pressure,temperature)
    implicit none
    real,parameter :: dep_change = 80 ! C
    real,parameter :: water_change = 200 ! C
    real,parameter :: d1=70 !km
    real :: pressure,temperature
    real :: depth,temperature1
      ...
      depth = pressure*1e+3/(3.2 * 9.8)
      if (depth .gt. d1) then
         temperature = temperature1 - water_change
      else
         temperature = temperature1 + dep_change
      end if
    ...
    subroutine simple_water(pressure,temperature)
    implicit none
    real,parameter :: dep_change = 0 ! C
    real,parameter :: water_change = 400 ! C
    real,parameter :: d1=70 !km
    real :: pressure,temperature
    real :: depth,temperature1
    ...
    subroutine unrelaxed_shear_modulus_issak_1992(temperature, pressure, shear_modulus)
      ...
      shear_modulus = Gur + (temperature + 273.) * dGudT + pressure * dGudP !Pa
    ...
    subroutine unrelaxed_shear_modulus(temperature,pressure,shear_modulus)
      ...
      !real, parameter :: dGudP = 1.7           ! pressure derivative Pa/Pa
      ...
      ! calculate unrelaxed modulus at given temperature and pressure
      shear_modulus = Gur + (temperature+273.)* dGudT + pressure * dGudP ! 5.11

      return
    end subroutine

!!Question: not PM2003 but 2012(in P&M 2013)
...
!!Question :Gur=66.5e+09(J&F 2010) bulk modulus?
    subroutine unrelaxed_shear_modulus_JF2010(temperature, pressure, shear_modulus, bulk_modulus)
      implicit none

      !=== Inverted from Jackson & Fual, PEPI, (2010)
      real, parameter :: Gur   = 66.5E+09  ! unrelaxed modulus (Pa)
      real, parameter :: dGudT = -13.6E+06  ! temp derivative Pa/degK
      real, parameter :: dGudP = 1.8        ! pressure derivative Pa/Pa

      real, parameter :: K0 = 129e+09       ! Bulk modulus(Pa)
      real, parameter :: dKdT = -0.016      ! Pa/K
      real, parameter :: dKdP = 4.6         ! Pa/Pa

      real :: temperature       ! deg C
      real :: pressure          ! Pa
      real :: shear_modulus     ! Pa
      real :: bulk_modulus      ! Pa
      ...
      bulk_modulus = K0 + (temperature + 273.) * dKdT + pressure * dKdP !!!why??

      return
    end subroutine

!!!Question:can't find the last equ
    ...
    subroutine shear_viscosity(temperature, pressure, premelt_coeff, viscosity)
      ...
      viscosity = eta_nomelt * premelt_coeff ! ! ! this is input
...
!!!Question:2        
    subroutine setupzeroage(zeroaget,zerocoeff,pottemp,adbatgrad,srate)
      ...
      ! upwelling rate for flat plate separation beneath axis 
      ! 2*srate/pi is from (Morgan and Forsyth 1988)
      ! Question: 3.15576E+10 ????
      ...
      do i = 1, 201
        z = (i-1)
        tzeroage(i) = zeroaget(i) - zeroaget(201)*z/bot!!transient temp 0 age
...
!!!Question: viscosity? qinv?vs?
    ...
    subroutine mccarthy_etal_2011(z,T,Vs,Q,eta)
      ...
      xn = 0.32*taun**taufac!!!taun>1e-11
...
!====== here choose a solidus you want=============================
      !call predotite_solidus_Hirschmann2000(P_gpa, Tm)
      call predotite_wetsolidus_Hirschmann2000(P_gpa, Tm)
      !call linear_depletion(P_gpa,Tm)
      !call predotite_solidus_Katz2003(P_gpa,Tm)
      !call simple_depletion(P_gpa,Tm)
      !call simple_water(P_gpa,Tm)      
      !call linear_fit(P_gpa,Tm)
      ...
      if (Tn < 0.91) then
          ...
          amp_p = 0.03  ! here ignored the direct influence of melt
      endif
...
!!!Question:water_exp & equ 10
    ...
    subroutine arrhenius_atten(depth, temperature, Q)!What's this?
      implicit none
